...
function Extract-Data {
    ...
    try {
        docker-compose -f docker-compose.training.yml run --rm transformer-training python -m app.data_extractor --output /app/data/training_data.json --min-unpredictability 0.2
...
function Train-Model {
    ...
    try {
        docker-compose -f docker-compose.training.yml run --rm transformer-training python -m app.train_transformer --epochs $Epochs --batch-size $BatchSize --model-name $ModelName --output-dir /app/models --print-every 10
...
function Test-Model {
    ...
    try {
        docker-compose -f docker-compose.training.yml run --rm transformer-training python -m app.predict_transformer /app/$ModelPath --sequence "010101"
    } catch {
...
function Start-Interactive {
    ...
    try {
        docker-compose -f docker-compose.training.yml run --rm transformer-training python -m app.predict_transformer /app/$ModelPath --interactive
    } catch {
...
function Benchmark-Model {
    ...
    try {
        docker-compose -f docker-compose.training.yml run --rm transformer-training python -m app.predict_transformer /app/$ModelPath --benchmark /app/data/training_data.json
    } catch {
...
switch ($Command.ToLower()) {
    ...
    "quick-train" {
        if (Check-Database) {
            ...
            docker-compose -f docker-compose.training.yml run --rm transformer-training python -m app.train_transformer --epochs 20 --batch-size 8 --d-model 64 --num-layers 3 --model-name quick_model --output-dir /app/models
        }
    }
    ...
    "advanced-train" {
        if (Check-Database) {
            ...
            docker-compose -f docker-compose.training.yml run --rm transformer-training python -m app.train_transformer --epochs 100 --batch-size 32 --d-model 256 --num-layers 8 --model-name advanced_model --output-dir /app/models
        }
    }
    ...
    "gpu-train" {
        if (Check-Database) {
            ...
            docker-compose -f docker-compose.training.yml --profile training-gpu run --rm transformer-training-gpu python -m app.train_transformer --device cuda --epochs $Epochs --batch-size $BatchSize --model-name gpu_model --output-dir /app/models
        }
    }
